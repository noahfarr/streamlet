...
from streamlet.optimizers import Implicit, Optimizer
...
class RecurrentQRCLambda:
    ...
    def __post_init__(self):
        ...
        assert isinstance(action_space, Discrete), (
            "RecurrentQRCLambda requires a discrete action space, got "
        ...
        assert self.cfg.regularization_coefficient >= 0.0, (
            "regularization_coefficient must be >= 0, got "
            f"{self.cfg.regularization_coefficient}."
        )
    ...
    def _update_step(
        ...
        if self.aux_h_loss is not None:
            ...
            h_params = jax.tree.map(lambda p, g: p - g, h_params, aux_h_grads)

        if isinstance(self.q_optimizer, Implicit):
            assert self.cfg.gradient_correction, (
                "RecurrentQRCLambda with the Implicit q-optimizer requires "
                "gradient_correction=True; the implicit step is derived for the "
                "full direction delta z - h g - e_h b."
            )
            rho_trace = self.q_optimizer.precondition(state.q_optimizer_state, q_trace)
            curvature = -sum(
                jnp.sum(b * z)
                for b, z in zip(jax.tree.leaves(td_grads), jax.tree.leaves(rho_trace))
            )
            ...
            q_updates = jax.tree.map(lambda td_g: -bias_trace * td_g, td_grads)
        ...
        bias_trace = jnp.where(
            transition.second.done | non_greedy,
