...
parser.add_argument(
    "--env-id",
    default="craftax::Craftax-Classic-Symbolic-v1",
    choices=[
        "craftax::Craftax-Classic-Symbolic-v1",
    ],
    help="Craftax Classic (symbolic) environment to train on.",
)
...
q_network = nn.Sequential(
    [
        nn.Dense(512, kernel_init=sparse_init),
        LayerNorm(),
        nn.leaky_relu,
        nn.Dense(512, kernel_init=sparse_init),
        LayerNorm(),
...
for i in range(num_epochs):
    ...
    data = {
        ...
        "training/normalized_episode_returns": episode_returns / max_return,
